...
type PostBody struct {
	...
	Location            string `json:"location,omitempty" validate:"omitempty,gte=1"`
...
func Post(userId uint64, post PostBody) (e Event, err error) {
	...
	result, err := mysql.Write(
		`INSERT INTO events (user_id, title, location, published, completed, auto_notify_documents_enabled)
			VALUES (?, ?, ?, ?, ?, ?)`,
		userId, post.Title, post.Location, post.Published, post.Completed, post.AutoNotifyDocuments,
	)
	if err != nil {
		return
	}
